...
path = '/home/neongreen13/Desktop/vids/'
...
for file in filelist[:]:
    ...
    if file.endswith(".mp4"):
        ...
        vid_name = file.split('.')
        os.mkdir(path + 'Worlds_2019/' + vid_name[0])
        os.chdir(path + 'Worlds_2019/' + vid_name[0] + '/')
        ...
        while True:
            ...
            if rounded[1][0] == '0':
                plt.imsave(str(timestamp) + '.png', sky, dpi=600)

                # Initial screen capture
                if len(score_changes) == 0:
                    score_changes.append(timestamp)
                    # resized = cv2.resize(sky,(660,590),fx=0,fy=0, interpolation = cv2.INTER_CUBIC)
                    # plt.imsave(str(timestamp) + '.png', resized, dpi=600)
                    plt.imsave(str(timestamp) + '.png', sky, dpi=600)
